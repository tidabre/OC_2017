	...
	public int getAction(Game game, long timeDue) {
		...
		if (flee) { // Weglaufen
			...
			for (int i = 0; i < Game.NUM_GHOSTS; i++) {
				if (game.getPathDistance(
						game.getCurPacManLoc(),
						game.getCurGhostLoc(i)) < distanceToGhost) {
					distanceToGhost = game.getPathDistance(
							game.getCurPacManLoc(),
			...
			for (int i = 0; i < neighbours.length; i++) {
				if (neighbours[i] < 0) { // Falls keine Zelle in dieser
											// Nachbarschaft ist
					continue;
				}
				final int tmpDistance = game.getPathDistance(
						neighbours[i],
						game.getCurGhostLoc(nextGhost));
			...
			final int[] targetsArray = new int[activePills.length
